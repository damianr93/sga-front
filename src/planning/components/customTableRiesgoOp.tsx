...
interface CustomTableProps {
  ...
  data: RowData[];
  actions?: Action[];
  criterioUmbral:Number
  onActionClick: (action: Action, row: any) => void;
  pagination?: PaginationProps;
}


const CustomTableRiesgosOportunidades: React.FC<CustomTableProps> = ({ columns, data, actions, criterioUmbral , onActionClick, pagination }) => {
              ...
              return (
                <TableRow key={rowIndex} sx={rowStyle}>
                  {columns.map((column) => {
                    ...
                    // Determinar si esta celda es "factorDeRiesgo" y necesita estilo especial
                    const isHighRiskFactor =
                      column.field === "factorDeRiesgo" &&
                      !isNaN(Number(value)) &&
                      Number(value) > +criterioUmbral;

                    return (
                      <TableCell
                        key={`${rowIndex}-${column.field}`}
                        align={column.align || "left"}
                        sx={isHighRiskFactor ? { bgcolor: '#ffebee', color: '#d32f2f', fontWeight: 'bold' } : {}}
                      >
                        {value}
                      </TableCell>
                    );
...
export default CustomTableRiesgosOportunidades;